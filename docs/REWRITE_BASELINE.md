...
Current active quick-gate semantic coverage is **32 samples / 176 cases**.
...
- `calc_fib` is `ci_skip` on `windows-latest` because the current hosted toolchain still emits a loop/codegen shape that fails lifting there even though local developer runs pass.
