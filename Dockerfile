...
RUN ["/bin/bash", "-c", "if [ \"$TESTING\" == \"false\" ]; then cp /root/Mergen/build/lifter /output/lifter; fi"]
# bash magic
ENTRYPOINT ["/bin/bash", "-c", "if [ \"$TESTING\" == \"true\" ]; then /root/Mergen/build/lifter fi"]