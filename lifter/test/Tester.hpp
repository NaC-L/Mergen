...
// retrieve_generalized_loop_phi_address_value_impl with byteCount=2
// returns an i16 phi carrying the masked low 16 bits of the loaded values.
// Complements the existing byteCount=1 coverage.
...
// the masked low byte of the local-slot loaded values. Complements the
// byteCount=2 local_phi_address test.
bool runGeneralizedLocalPhiAddressByteCountOneReturnsMaskedPhi(
  ...
  constexpr uint64_t canonicalControl = 0x1401AF740ULL;
  constexpr uint64_t backedgeControl = 0x1401AF0F6ULL;
  constexpr uint64_t localA = STACKP_VALUE + 88;
  constexpr uint64_t localB = STACKP_VALUE + 96;
  ...
  if (!sawA || !sawB) {
    details = "  local_phi_address byteCount=1 should carry masked low-byte loaded values\n";
    return false;
  }
  return true;
}
